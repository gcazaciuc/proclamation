(function (global, factory) {
	...
	const DragAndDrop = (el, spec) => {
	    dragula_1([el]);
	};
	DragAndDrop.registeredProps = ['drag-axis', 'draggable'];
	...
	const State = createBehaviour(Binders);
	...
	function runValidations$1(el, validations) {
	    const validationKeys = Object.keys(validator);
	    console.log(validationKeys);
	...
	const Validations = createBehaviour(Binders$1);

	const availableBehaviours = [DragAndDrop, State, Ajax, Validations];
	...
	function applyBehavioursToEl(behaviours) {
	    return el => {
	        behaviours.forEach(b => applyBehaviour(el, b));
	    };
	}

	function applyAllBehaviours() {
	    Object.keys(behaviours).forEach(selector => {
	        const elBehaviours = behaviours[selector];
	        Array.from(document.querySelectorAll(selector)).forEach(applyBehavioursToEl(elBehaviours));
	    });
	}

	function behave(selector, behaviourSpec) {
	    behaviours[selector] = behaviours[selector] || [];
	    behaviours[selector].push(behaviourSpec);
	}
	...
	function flattenBehaviours(behaviours) {
	    behaviours.forEach(behaviour => {
	        parseBehaviourSpec(behaviour);
	    });
	}

	function domMutated(mutation) {
	        ...
	        case 'childList':
	            ...
	            mutation.addedNodes.forEach(el => {
	                ...
	                applicableSelectors.forEach(s => {
	                    ...
	                    elBehaviours.forEach(b => applyBehaviour(el, b));
	                });
	            });
	            break;
	    }
	...
	const install = () => {
	    window.addEventListener('load', () => {
	        ...
	        flattenBehaviours(parsedBehavioursSpec);
	        applyAllBehaviours();
	        watchDocument();
